...
urlpatterns = [
    ...
    path('sign-up/verify/', views.VerifyEmailView.as_view(), name='sign-up-verify'),  # Эта API вьюшка будет
    # проверять ключ, через нее будем передавать ключ post запросом.
