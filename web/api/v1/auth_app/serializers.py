...
error_messages = {
    ...
    'not_active': _('Your account is not active. Check your email.'),
...
class UserSignUpSerializer(serializers.Serializer):
    ...
    def validate_email(self, email: str) -> str:
        if AuthAppService.is_user_exist(email):
            raise serializers.ValidationError(_(error_messages['already_registered']))
        return email
...
class VerifyEmailSerializer(serializers.Serializer):
    key = serializers.CharField()  # Здесь все таки будет key! (был token) Тот самый Key который сгенерировался для
    # подтверждения email из id пользователя.
